...
(function (global) {
  ...
  const PALETTES = {
    light: {
      deep: [0.03, 0.54, 0.37],
      bright: [0.0, 0.92, 0.46],
      accent: [0.05, 0.78, 0.62],
      line: [0.04, 0.82, 0.63],
      fog: 0xf4fbf7,
      fogDensity: 0.00105,
      ambient: 0xc8f5dc,
    ...
    dark: {
      deep: [0.08, 0.42, 0.28],
      bright: [0.39, 1.0, 0.78],
      accent: [0.24, 1.0, 0.67],
      line: [0.31, 1.0, 0.78],
      fog: 0x060d07,
      fogDensity: 0.00135,
      ambient: 0x1a3d28,
  ...
  function createNodeMaterial(THREE, pal) {
    return new THREE.ShaderMaterial({
      ...
      vertexShader: [
        ...
        "  vColor = lit;",
        "  vAlpha = clamp(1.15 - dist / 820.0, 0.38, 1.0);",
        "  float size = (4.8 + aActivity * 3.6) * uPixelRatio * (300.0 / dist);",
  ...
  function run(THREE, canvas, section) {
    ...
    const ambient = new THREE.AmbientLight(pal.ambient, getTheme() === "dark" ? 0.42 : 0.28);
    scene.add(ambient);

    const keyLight = new THREE.PointLight(pal.point, getTheme() === "dark" ? 0.95 : 0.62, 1400, 1.6);
    keyLight.position.set(120, 80, 360);
    scene.add(keyLight);

    const fillLight = new THREE.PointLight(0x8affc8, getTheme() === "dark" ? 0.35 : 0.22, 1100, 2);
    ...
    const applyThemeVisuals = () => {
      ...
      ambient.intensity = theme === "dark" ? 0.42 : 0.28;
      keyLight.color.setHex(pal.point);
      keyLight.intensity = theme === "dark" ? 0.95 : 0.62;
    ...
    const tick = () => {
      ...
      if (nextTheme !== theme) applyThemeVisuals();
