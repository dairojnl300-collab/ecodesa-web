...
(function (global) {
  ...
  const LAYERS = [
    { speed: 0.42, sizeMul: 1.08, alpha: 0.72, glow: 0.72 },
    { speed: 0.72, sizeMul: 1.24, alpha: 0.86, glow: 0.9 },
    { speed: 1.12, sizeMul: 1.42, alpha: 0.96, glow: 1.08 }
  ];
  ...
  const PALETTES = {
    light: {
      ...
      fogDensity: 0.00032,
      ambient: 0xe8f5ef,
      lineAlphaIntra: [0.42, 0.62],
      lineAlphaBridge: [0.14, 0.28],
      nodeGlow: 0.92,
      exposure: 1.14,
      blending: "normal"
    },
    dark: {
      ...
      fogDensity: 0.00055,
      ambient: 0x0a1810,
      lineAlphaIntra: [0.34, 0.56],
      lineAlphaBridge: [0.1, 0.22],
      nodeGlow: 1.38,
      exposure: 1.2,
      blending: "additive"
    }
  ...
  function createNodeMaterial(THREE, pal) {
    ...
    return new THREE.ShaderMaterial({
      uniforms: {
        ...
        uSizeMul: { value: isDark ? 1.34 : 1.24 }
      },
      vertexShader: [
        ...
        "  vAlpha = aLayerAlpha * clamp(1.28 - dist / 980.0, 0.5, 1.0);",
        "  float size = (12.0 + aActivity * 9.0) * aLayerSize * uSizeMul * uPixelRatio * (360.0 / dist);",
        "  gl_PointSize = clamp(size, 8.0, 46.0);",
      ...
      fragmentShader: [
        ...
        "  float inner = exp(-d * d * 7.0) * 0.72;",
        "  float halo = smoothstep(0.5, 0.02, d) * vGlow * 1.12;",
        "  vec3 col = vColor * (core + inner * 0.62) + vColor * halo * 1.08;",
        "  col += vec3(1.0) * core * 0.24;",
        "  float alpha = clamp((core + inner * 0.42 + halo * 0.82) * vAlpha, 0.0, 1.0);",
  ...
  function run(THREE, canvas, section) {
    ...
    const TUBE_RADIUS = 0.9;
    const tubeGeo = new THREE.CylinderGeometry(1, 1, 1, 6, 1, true);
    tubeGeo.setAttribute("instanceAlpha", new THREE.InstancedBufferAttribute(new Float32Array(MAX_LINKS), 1));
    ...
    const tubeMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: [
        ...
        "varying vec3 vColor;",
        "void main() {",
        "  float edgeFade = smoothstep(0.0, 0.18, vFade) * (1.0 - smoothstep(0.82, 1.0, vFade));",
    ...
    const tubes = new THREE.InstancedMesh(tubeGeo, tubeMat, MAX_LINKS);
    tubes.instanceColor = new THREE.InstancedBufferAttribute(new Float32Array(MAX_LINKS * 3), 3);
    tubes.count = 0;
    ...
    const applyThemeVisuals = () => {
      ...
      pointsMat.uniforms.uSizeMul.value = theme === "dark" ? 1.34 : 1.24;
      pointsMat.blending = theme === "dark" ? THREE.AdditiveBlending : THREE.NormalBlending;
      pointsMat.needsUpdate = true;
      tubeMat.blending = theme === "dark" ? THREE.AdditiveBlending : THREE.NormalBlending;
    ...
    const updateLines = (now) => {
      ...
      for (let i = 0; i < COUNT && linkCount < MAX_LINKS; i++) {
        ...
        for (let j = i + 1; j < COUNT && linkCount < MAX_LINKS; j++) {
          ...
          _dir.set(-dx, -dy, -dz).divideScalar(dist);
          _mid.set((a.x + b.x) * 0.5, (a.y + b.y) * 0.5, (a.z + b.z) * 0.5);
          _quat.setFromUnitVectors(_up, _dir);
          _scale.set(TUBE_RADIUS, dist, TUBE_RADIUS);
          _matrix.compose(_mid, _quat, _scale);

          tubes.setMatrixAt(linkCount, _matrix);
          _tubeColor.setRGB(cr, cg, cb);
          tubes.setColorAt(linkCount, _tubeColor);
          tubeGeo.attributes.instanceAlpha.setX(linkCount, midAlpha);

          linkCount++;
        }
      }

      tubes.count = linkCount;
      tubes.instanceMatrix.needsUpdate = true;
      tubes.instanceColor.needsUpdate = true;
      tubeGeo.attributes.instanceAlpha.needsUpdate = true;
    };
    ...
    const tick = () => {
      ...
      if (!reducedMotion) pointsGeo.attributes.position.needsUpdate = true;
      ...
      if (!reducedMotion) {
        ...
        camera.position.z = 500 - scrollT * 28;
        camera.lookAt(0, 0, 0);
      }
